...
async def test_create_patient(client, doctor_token):
    patient_data = {
        "given_name": "Ama", "family_name": "Doe",
        "date_of_birth": "1990-01-01",
        "gender": "female", "phone": "+228 90 00 00 00", "locale": "fr",
    }
    mock_doc = {
        "_id": "p1", "fhir_id": "f1", **patient_data,
        "name_text": "Ama Doe", "status": "active",
    }
    with patch(
        "app.services.patient_service.create_patient",
        new_callable=AsyncMock, return_value=mock_doc,
    ):
        resp = await client.post(
            "/api/v1/patients", json=patient_data,
...
async def test_get_patient_not_found(client, doctor_token):
    with patch(
        "app.services.patient_service.get_patient",
