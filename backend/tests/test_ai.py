...
async def test_ai_query_high_confidence(client, doctor_token):
    with (
        patch("app.api.routes.ai.ai_service.query", new_callable=AsyncMock, return_value=make_ai_response(0.9)),
        patch("app.api.routes.ai.log_audit", new_callable=AsyncMock),
    ):
...
async def test_ai_query_low_confidence_warning(client, doctor_token):
    with (
        patch("app.api.routes.ai.ai_service.query", new_callable=AsyncMock, return_value=make_ai_response(0.4)),
        patch("app.api.routes.ai.log_audit", new_callable=AsyncMock),
    ):
        resp = await client.post(
            "/api/v1/ai/query",
...
async def test_ai_differential_doctor_allowed(client, doctor_token):
    with (
        patch("app.api.routes.ai.ai_service.differential", new_callable=AsyncMock, return_value=make_ai_response()),
        patch("app.api.routes.ai.log_audit", new_callable=AsyncMock),
    ):
        resp = await client.post(
...
async def test_ai_literature_search(client, doctor_token):
    with (
        patch("app.api.routes.ai.ai_service.literature", new_callable=AsyncMock, return_value=make_ai_response()),
        patch("app.api.routes.ai.log_audit", new_callable=AsyncMock),
    ):
        resp = await client.post(
